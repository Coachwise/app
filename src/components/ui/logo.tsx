...
/**
 * The Coachwise mark: a climber cresting a ridge inside a rising ring.
 *
...
interface BrandProps {
  /** The product name in the reader's script — pass t('appName'). */
  name: string;
  /** Wordmark under the mark (default) or beside it. */
  layout?: 'stacked' | 'inline';
  /** The plate the mark sits on. 'none' lets the mark inherit the text colour. */
  tile?: BrandTile;
  size?: BrandSize;
  className?: string;
}
...
const SIZE: Record<BrandSize, { plate: string; mark: string; name: string; gap: string }> = {
  sm: { plate: 'w-14 h-14 rounded-2xl', mark: 'w-10 h-10', name: 'text-2xl', gap: 'gap-2' },
  lg: { plate: 'w-28 h-28 rounded-3xl', mark: 'w-20 h-20', name: 'text-4xl', gap: 'gap-3' },
};

/**
 * The brand lockup: the mark with the wordmark held close to it. The wordmark is
 * set in the display face — it is the only place that face is used, which is what
 * makes it read as the brand rather than as a heading.
 *
 * The name is passed in rather than hardcoded, because it is translated
 * (Coachwise / کوچ‌وایز) like everything else on the screen.
 */
export function Brand({ name, layout = 'stacked', tile = 'none', size = 'lg', className }: BrandProps) {
  const t = TILE[tile];
  const s = SIZE[size];
  ...
  return (
    ...
    >
      {tile === 'none' ? (
        <Logo className={cn(s.mark, t.mark)} />
      ) : (
        <span className={cn('inline-flex items-center justify-center shrink-0', s.plate, t.plate)}>
          <Logo className={cn(s.mark, t.mark)} />
        </span>
      )}
      <span className={cn('font-display leading-none', s.name)}>{name}</span>
    </div>
  );
}
