...
export function beep(freq = 880, ms = 120, gain = 0.18) {
  if (!ctx) initAudio();
  if (!ctx) return;
  try {
...
function play(notes: Note[], type: OscillatorType = 'triangle') {
  ...
  if (!ctx) return;
  const now = ctx.currentTime;
  for (const [freq, atMs, durMs, gain = 0.18] of notes) {
    try {
      const osc = ctx.createOscillator();
      const vol = ctx.createGain();
      osc.type = type;
...
export const soundStartRest = () => play([[587, 0, 130, 0.15], [392, 120, 200, 0.15]], 'sine');
...
// Bigger fanfare at the end of the whole plan.
export const soundPlanDone = () =>
  play([
    [523, 0, 120], [659, 110, 120], [784, 220, 120], [1047, 330, 180],
    [1047, 540, 340, 0.2], [1319, 540, 340, 0.17], [1568, 540, 420, 0.15],
  ]);
